...
export default function TransactionsPage() {
  ...
  const [pendingRequests, setPendingRequests] = useState<PendingMaterialRequest[]>([]);
  ...
  async function loadPendingRequests() {
    const response = await fetch("/api/transfers?status=REQUESTED&limit=100", {
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      setError(json.error ?? "Failed to load pending material requests.");
      return;
    }
  ...
  useEffect(() => {
    Promise.all([loadTransactions(), loadLookups(), loadPendingRequests()]).catch(() =>
    ...
    <div className="space-y-6">
      ...
      <Card className="min-h-[24rem]">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-lg font-semibold">Pending Material Requests</h2>
        ...
        <div className="mt-4 max-h-[24rem] overflow-auto">
          <table className="ims-table">
            ...
            <tbody>
              {pendingRequests.map((request) => {
                const line = request.transfer_lines?.[0];
                const fromLocation = locationById.get(request.from_location_id);
                const toLocation = locationById.get(request.to_location_id);
                const product = line ? productById.get(line.product_id) : null;
                return (
                  <tr key={request.id} className="ims-table-row">
                    <td className="font-medium">{request.transfer_number}</td>
                    <td>
                      {fromLocation
                        ? `${fromLocation.code ?? "LOC"} - ${fromLocation.name}`
                        : request.from_location_id}
                    </td>
                    <td>
                      {toLocation
                        ? `${toLocation.code ?? "LOC"} - ${toLocation.name}`
          ...
          {pendingRequests.length === 0 ? (
            <p className="ims-empty mt-3">No pending material requests.</p>
          ) : null}
        </div>
      </Card>
